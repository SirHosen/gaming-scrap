#!/usr/bin/env python3
"""
Central configuration for the NESTfetch scraper.
...
# ── Ethics: robots.txt politeness ───────────────────────────
# When True, the engine consults each site's robots.txt and skips any URL the
# site asks bots not to crawl. robots.txt itself is always fetched. If a site's
# robots.txt is unreachable, we fail open (allow) but log at debug level.
RESPECT_ROBOTS_TXT: bool = True
# User-agent token matched against robots.txt rules ("*" = the catch-all group).
ROBOTS_USER_AGENT: str = "*"
# Per-host timeout (seconds) when fetching robots.txt.
