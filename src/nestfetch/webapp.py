...
try:  # non-secret structural defaults live in config.py
    from nestfetch.config import WEB_DEFAULT_HOST, WEB_DEFAULT_PORT
...
def sites_payload() -> list:
    ...
    from nestfetch.sites.registry import available_sites
    return [
        {
            "name": m.name,
            "platform": m.platform,
...
def scrape_params_from(data: dict, db_path: Optional[str]) -> dict:
    ...
    from nestfetch.config import CACHE_ENABLED_DEFAULT, PER_HOST_RATE_LIMIT
    return {
    ...
    *,
    ...
    if open_browser:
        try:
            ...
            # Opening a browser is a convenience, never required for the server.
...
<header>
  <div>
    ...
    <div class="sub">Multi-site game-download scraper &middot; v4.9</div>
  </div>
